...
const searchStock = async () => {
  ...
  try {
    // 获取股票基本信息
    const infoResponse = await axios.get(`/api/abu/stock/${stockSymbol.value.trim()}`)
    stockInfo.value = infoResponse.data

    // 获取股票历史数据
    const historyResponse = await axios.get(`/api/abu/stock/${stockSymbol.value.trim()}/history`)
